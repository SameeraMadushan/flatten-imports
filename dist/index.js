...
program
    ...
    .description("Codemod to replace named imports from barrel files with direct imports")
    .option("-p, --paths <paths>", "Comma-separated list of barrel directories (e.g. src/components,src/hooks)", "src/components")
...
const isTargetImport = (importPath) => {
    return barrelDirs.some((dir) => importPath === `${aliasPrefix}${dir.replace(/^src\//, "")}` ||
...
project.getSourceFiles().forEach((sourceFile) => {
    ...
    if (didChange) {
        totalFilesTouched++;
        console.log(`${dryRun ? "[Dry Run] " : ""}✔ Updated: ${sourceFile.getFilePath()}`);
        if (!dryRun)
            sourceFile.saveSync();
    }
});
